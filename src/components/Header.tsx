        ...
        <div className="ml-auto hidden items-center gap-2 md:flex" data-tutorial="header-account">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              {isAuthenticated ? (
                <button className="flex items-center gap-2 rounded-full border border-stone-200 bg-white py-1.5 pl-1.5 pr-3 text-sm font-semibold text-stone-800 transition hover:border-brand/30"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-brand text-xs font-bold text-white">{initial}</span>MY<ChevronDown className="h-3.5 w-3.5 text-stone-400" /></button>
              ) : (
                <button className="flex items-center gap-1 rounded-full px-4 py-2 text-sm font-semibold text-stone-600 transition hover:bg-stone-100 hover:text-stone-950">MY<ChevronDown className="h-3.5 w-3.5" /></button>
              )}
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-60 rounded-2xl p-2">
              {isAuthenticated ? (
                <>
                  <DropdownMenuLabel className="px-2 py-1.5"><p className="text-sm font-bold text-stone-950">{displayName}</p>{identity.brandName && <p className="text-xs font-normal text-stone-500">@{identity.nickname}</p>}</DropdownMenuLabel>
                  <DropdownMenuSeparator />
                  {myItems.map((item) => <DropdownMenuItem key={`${item.to}-${item.label}`} asChild className="cursor-pointer rounded-xl py-2 text-sm font-semibold"><Link to={item.to}>{item.label}</Link></DropdownMenuItem>)}
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={handleSignOut} className="cursor-pointer rounded-xl py-2 text-sm font-semibold text-stone-500"><LogOut className="mr-2 h-4 w-4" /> 로그아웃</DropdownMenuItem>
                </>
              ) : (
                <><DropdownMenuItem asChild className="cursor-pointer rounded-xl py-2 text-sm font-semibold"><Link to="/auth?mode=login">로그인</Link></DropdownMenuItem><DropdownMenuItem asChild className="cursor-pointer rounded-xl py-2 text-sm font-semibold"><Link to="/auth?mode=signup">회원가입</Link></DropdownMenuItem></>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
