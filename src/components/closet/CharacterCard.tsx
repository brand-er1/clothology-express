...
export const CharacterCard = ({ gender, selected, onSelect }: CharacterCardProps) => {
  ...
  return (
    <Card
      className={`flex flex-col items-center overflow-hidden rounded-xl border-2 p-6 text-center shadow-sm transition ${
        selected ? "border-brand bg-brand/5" : "border-stone-200 bg-white hover:border-brand/40"
      }`}
    >
      <div className="flex h-56 w-full items-center justify-center overflow-hidden rounded-lg bg-[#f4f0ea]">
        <img
