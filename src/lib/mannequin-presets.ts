import { getMannequinSvgDataUrl } from "@/lib/mannequin-figure";
import type { CharacterGender, FemaleMannequinSize, MaleMannequinSize, MannequinSize } from "@/types/closet";

export const femaleMannequinSizes: FemaleMannequinSize[] = ["44", "55", "66", "77"];
...
export const mannequinBodyDescription: Record<MannequinSize, string> = {
  ...
  "77": "a fuller female body preset: noticeably more volume through shoulders, chest, waist, hip, arms and legs than the 66 preset, still a natural, realistic body — never exaggerated or unrealistic",
...
export interface MannequinPreset {
  ...
  size: MannequinSize;
  label: string;
  /** Vector "fashion croquis" placeholder shown before any AI fitting exists, and rasterized as the AI reference image. */
  previewImage: string;
...
const buildPreset = (gender: CharacterGender, size: MannequinSize): MannequinPreset => ({
  ...
  previewImage: getMannequinSvgDataUrl(gender, size),
 ...
 * NOTE: previewImage is currently a generated vector placeholder (see mannequin-figure.ts) — replace
 * with a real reviewed reference photo/render URL per key once BRAND-ER supplies one; no other code
 * needs to change, every caller resolves the reference image through this registry.
 */
...
/**
 * Face Identity Reference image per gender. Today this is the same generated croquis face used in the
 * mannequin preset itself (so "face" and "mannequin" reference are pixel-consistent by construction).
 * BRAND-ER plans to supply a dedicated real face reference photo per gender later ("추후 별도로
 * 제공") — swap this to return that URL and the rest of the pipeline (which always sends face
 * reference + mannequin reference together) needs no change.
 */
export const getFaceReferenceImage = (gender: CharacterGender): string =>
  getMannequinPreset(gender, defaultMannequinSize(gender)).previewImage;
