  ...
  <Link to={`/community/${post.id}`} className="group block min-w-0">
    <div className="relative aspect-[4/5] w-full overflow-hidden rounded-2xl bg-stone-100">
      ...
      {post.fundingStatus !== "none" && (
        <span className="absolute left-2 top-2 rounded-full bg-brand px-2.5 py-1 text-[10px] font-bold text-white shadow">
          🔥 {COMMUNITY_FUNDING_STATUS_LABEL[post.fundingStatus]}
        </span>
      )}
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent px-3 pb-2.5 pt-8">
        <div className="flex items-center gap-3 text-[11px] font-semibold text-white/95">
          <span className="flex items-center gap-1">
            <Heart className={`h-3.5 w-3.5 ${post.likedByMe ? "fill-white" : ""}`} /> {post.likeCount}
          </span>
          <span className="flex items-center gap-1">
            <MessageCircle className="h-3.5 w-3.5" /> {post.commentCount}
          </span>
          <span className="flex items-center gap-1">
            <Flame className={`h-3.5 w-3.5 ${post.purchaseIntentByMe ? "fill-white" : ""}`} /> {post.purchaseIntentCount}명
          </span>
        </div>
      </div>
    </div>
...
const CommunityFeed = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f7f6f4]">
      ...
      <main className="mx-auto max-w-[1100px] px-4 pb-24 pt-20 sm:px-6 sm:pt-24">
        <section className="relative pt-4 text-center sm:pt-6">
          <Link
            ...
            className="absolute right-0 top-4 flex h-9 w-9 items-center justify-center rounded-full bg-white text-stone-500 shadow-sm hover:text-brand sm:top-6"
            aria-label="알림"
          >
        ...
        <section className="mx-auto mt-6 max-w-xl">
          <div className="relative">
            ...
            <Input
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter") handleSearchSubmit(searchInput.trim());
              }}
              placeholder="디자인, 브랜드, 사용자, 해시태그 검색"
              className="h-12 rounded-full border-stone-200 bg-white pl-10 pr-10 shadow-sm"
            />
            {searchInput && (
              <button
                type="button"
          ...
          <div className="mt-3 flex flex-wrap justify-center gap-2">
            {POPULAR_HASHTAGS.map((tag) => (
              <button
                key={tag}
                type="button"
                onClick={() => { setSearchInput(`#${tag}`); handleSearchSubmit(tag); }}
                className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-stone-600 shadow-sm transition hover:bg-brand/10 hover:text-brand"
              >
                #{tag}
              </button>
            ))}
            ...
            {filterTabs.map((tab) => (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold transition ${
                  filter === tab.value ? "bg-brand text-white" : "bg-white text-stone-500 hover:text-stone-900"
                }`}
              >
                {tab.label}
              </button>
        ...
        {activeSearch && (
          <p className="mt-4 text-sm text-stone-500">
            "<span className="font-semibold text-stone-800">{activeSearch}</span>" 검색 결과 {posts.length}건
          </p>
        )}
        ...
        <section className="mt-4">
          {isLoading ? (
            <div className="flex h-64 items-center justify-center text-sm text-stone-500">
              <Loader2 className="mr-2 h-5 w-5 animate-spin text-brand" /> 게시물을 불러오는 중입니다
            </div>
          ) : posts.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-stone-300 bg-white/60 py-24 text-center">
              <p className="text-lg font-bold text-stone-700">아직 게시물이 없습니다.</p>
              <p className="mt-2 text-sm text-stone-500">가장 먼저 당신의 디자인을 공유해보세요.</p>
              <Button asChild className="mt-6 rounded-full bg-brand hover:bg-brand-dark">
                <Link to="/community/new">새 디자인 공유하기</Link>
              </Button>
            </div>
          ) : (
            <>
      ...
      <Link
        to="/community/new"
        className="fixed bottom-20 right-5 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-brand text-white shadow-xl transition hover:bg-brand-dark sm:bottom-8"
        aria-label="새 디자인 공유하기"
      >
