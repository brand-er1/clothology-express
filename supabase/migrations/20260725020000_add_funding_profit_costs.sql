alter table public.fundings
  add column if not exists estimate_direct_unit_min integer
    check (estimate_direct_unit_min is null or estimate_direct_unit_min >= 0),
  add column if not exists estimate_direct_unit_max integer
    check (estimate_direct_unit_max is null or estimate_direct_unit_max >= 0),
  add column if not exists estimate_development_total integer
    check (estimate_development_total is null or estimate_development_total >= 0),
  add column if not exists fabric_unit_cost integer not null default 0
    check (fabric_unit_cost >= 0);
